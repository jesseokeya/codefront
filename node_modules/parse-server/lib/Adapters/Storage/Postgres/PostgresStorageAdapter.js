...
class PostgresStorageAdapter {
  ...
  setIndexesWithSchemaFormat(className, submittedIndexes, existingIndexes = {}, fields, conn) {
    ...
    return conn.task(t => {
      ...
      return t.batch([deletePromise, insertPromise, this._ensureSchemaCollectionExists(t), t.none('UPDATE "_SCHEMA" SET $2:name = json_object_set_key($2:name, $3::text, $4::jsonb) WHERE "className"=$1', values)]);
    });
  }
  ...
  createTable(className, schema, conn) {
    ...
    return conn.task(t => {
      return this._ensureSchemaCollectionExists(t).then(() => conn.none(qs, values)).catch(error => {
